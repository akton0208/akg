...
async def login_and_get_user_id(email, password, user_id=None):
    """Login using email and password to get user_id and access_token."""
    if user_id:
        return user_id, None, logger.bind(email=email)  # Directly return user_id if provided
...
async def send_ping(websocket, user_logger):
    ...
    while True:
        try:
            ...
            await asyncio.sleep(random.randint(59, 60))  # Shortened interval to 2-3 seconds
...
async def main():
    ...
    tasks = []
    for email, password, user_id in zip(emails, passwords, user_ids):
        user_id, access_token, user_logger = await login_and_get_user_id(email, password, user_id)
